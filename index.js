...
// ready function is used to check the HTML documnet is loaded completely
...
$(function(){
    $(".input_field").keyup(function(){
        let id= $(this).attr("id")
        let input_numbers=id.split("-")[1]
        $(".rep_input").eq(input_numbers).html($)
    })
...
function displayStory() {
  ...
  //make sure that the input box are empty before loading
  $("#input_fields").empty();

  //set the input boxes
  for(let i=0;i<story.inputs;i+=1){
      let input_html=`<input type="text" class="input_field" id="input_${i}" 
      placeholder="Input ${i+1}"></input>`
  ...
  for (var i=0; i<story.words.length; i+=1){
